...
--for row in DB.Query("SELECT ID, RangedCombat FROM Units WHERE RangedCombat > 0 AND Cost > 0") do
	--rCombatStrenghts[row.ID] = row.RangedCombat;
--end
end

function TViribusUnitis(PlayerID, unitID)
	...
	print("mon id est")
		...
		print("Promo verif ok")
			...
			local baseCombatStrength = tCombatStrengths[unit:GetUnitType()]
				...
				print("melee1 ok")
					unit:SetBaseCombatStrength(math.max(baseCombatStrength,baseCombatStrength + bonusPerConnection*NbCityConnected()))
					print("melee ok")
